...
//        tvTextSize.setText(String.valueOf(config.textSize));
        ...
        sbMaxHeight.setProgress(config.maxHeight);
...
//        sbTransY.setProgress(config.transY + 400);
     ...
     * 最大高度
     */
    private SeekBar sbMaxHeight;
    private TextView tvMaxHeight;

    private void initMaxHeightModule() {
        sbMaxHeight = findViewById(R.id.sb_max_height);
        tvMaxHeight = findViewById(R.id.tv_max_height);


        sbMaxHeight.setMax(500);
        sbMaxHeight.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                config.maxHeight = progress;
                tvMaxHeight.setText(String.valueOf(progress));
                notifyWordViewConfigChanged();
            }
